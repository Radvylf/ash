var interpret = (program = [], inputs = []) => {
    var code_page = [
        ...
        "⁽", "⁾", "₀", "₁", "₂", "₃", "₄", "₅", "₆", "₇", "₈", "₉", "₊", "₋", "₍", "₎",
        "Ɓ", "Ƈ", "Ɗ", "Ƒ", "Ɠ", "Ƙ", "Ɲ", "Ƥ", "Ƭ", "Ʋ", "Ⱳ", "Ȥ", "⅛", "¼", "½", "¾",
        "ɓ", "ƈ", "ɗ", "ɠ", "ɦ", "ƙ", "ɲ", "ƥ", "ʠ", "ƭ", "ⱳ", "ȥ", "‘", "’", "“", "”"
    ];
    ...
    var dynamic_operators = ["Ạ", "Ḅ", "Ḍ", "Ẹ", "Ḥ", "Ị", "Ḳ", "Ḷ", "Ṃ", "Ṇ", "Ọ", "Ṛ", "Ṣ", "Ṭ", "Ụ", "Ṿ", "Ẉ", "Ỵ", "Ẓ"];
    ...
    var is_integer = (data) => (typeof data == "bigint") ? 1n : 0n;
    var is_float = (data) => (typeof data == "number") ? 1n : 0n;
    var is_string = (data) => (typeof data == "string") ? 1n : 0n;
    var is_array = (data) => Array.isArray(data) ? 1n : 0n;
    ...
    var internal_divide = (...floats) => {
        var final_sign = sign(floats[0] ? floats[0] : 1 / floats[0]) * sign(floats[1] ? floats[1] : 1 / floats[1]);
        ...
        return (positives[0] / positives[1]) * final_sign;
    };
    ...
    var integer_divide = (...numbers) => to_integer(float_divide(...numbers));
    var floored_division = (...numbers) => take_type(Math.floor(float_divide(...numbers)), numbers[0]);
    ...
    var rational_power = (...numbers) => root(power(numbers[0], numbers[1]), numbers[2]);
    var float_power = (...numbers) => {
        var floats = [positive(to_float(numbers[0])), to_float(numbers[1])];

        if (floats[0] == 1 || floats[1] == 0)
            return 1;

        if (floats[0] != 0 && floats[0] != Infinity && floats[1] != -Infinity && floats[1] != Infinity)
            return floats[0] ** floats[1];

        return [0, Infinity][(sign(-(1 - floats[0])) * sign(floats[1]) + 1) / 2];
    };
    ...
    var range = (...bounds) => {
        ...
        if (is_integer(bounds[0])) {
            ...
            return array.map((_, i) => bounds[0] + to_integer(i) * step);
        } else {
            var array = [...Array(Math.ceil(Math.abs(bounds[0] - bounds[1])))];
            var step = sign(bounds[1] - bounds[0]);
    ...
    var round_to_n = (number, point) => {
        ...
        if (is_integer(number))
            return point < 0 ? (number / (10n ** -point) + sign(number) * to_integer(positive(number) / (10n ** -(point + 1n)) % 10n >= 5n)) * (10n ** -point) : number;
        else
    ...
    var write_digit = (number, position, digit) => {
        ...
        if (is_integer(number))
            ...
            return number - (Math.trunc((number / (10 ** position)) % 10) - (sign(number) || 1) * digit) * (10 ** position);
    };
    ...
    var unsigned_addition = (...numbers) => {
        ...
        if (is_integer(numbers[0]))
            ...
            return numbers.reduce((a, n, i) => i > 0n ? a + n * (sign(numbers[0]) || 1) : a, numbers[0]);
    };
    ...
    var to_base = (number, base) => {
        ...
        if (is_integer(number)) {
            ...
            while (number) {
                result = base[number % to_integer(base.length)][0] + result;
                number = number / to_integer(base.length);
            }
            ...
            return (sign(original) < 0 ? "-" : "") + (trim ? result.slice(0, -trim) : result);
        }
    };
    ...
    var from_base = (string, base) => {
        ...
        if (!input.includes(".")) {
            ...
            while (input) {
                number = number * to_integer(base.length) + to_integer(base.findIndex(n => n.includes(input[0])) + 1 || 1) - 1n;
                input = input.slice(1);
            }
    ...
    var bitwise_not = (number, bits) => {
        ...
        if (bits === undefined)
            numbers[1] = 2n ** (logarithm(numbers[0], 2n) + to_integer(numbers[0] != 0n));
        else
    ...
    var pad = (data, length, padding) => {
        var pad_width = positive(length) - data.length;

        if (pad_width < 0)
            return length >= 0 ? data.slice(0, length) : data.slice(length);

        if (is_array(data)) {
            var pad_with = multiply_all(to_array(padding), Math.max(Math.ceil(pad_width / to_array(padding).length), 0)).slice(0, pad_width);

            return length >= 0 ? array_concat(data, pad_with) : array_concat(reverse(pad_with), data);
        } else {
            var pad_with = multiply_all(to_string(padding), Math.max(Math.ceil(pad_width / to_string(padding).length), 0)).slice(0, pad_width);

            return length >= 0 ? string_concat(data, pad_with) : string_concat(reverse(pad_with), data);
        }
    };

    var zero_pad_start = (string, length) => pad(string, -Math.max(length, string.length), "0");
    ...
    var greatest_common_divisor = (...numbers) => {
        ...
        return to_integer(product(intersection(...numbers.map(n => prime_factors(n)))));
    };

    var least_common_multiple = (...numbers) => {
        numbers = numbers.map(to_integer);

        return to_integer(product(union(...numbers.map(n => prime_factors(n)))));
    };
    ...
    var take_case = (string, from) => {
        from = pad(to_string(from), string.length, " ");
    ...
    var partition_half = (data) => partition(data, Math.ceil(data.length / 2));
    
    var transpose = (array) => {
        array = array.map(to_array);
        
        var max = Math.max(...array.map(a => a.length));
        
        return range(0, max).map(i => array.map(d => i in d ? d[i] : null).filter(d => d !== null));
    };
    ...
    var combinations = (data) => {
        ...
        for (var j, i = 0n; i < 2n ** to_integer(array.length); i++) {
            result.push([]);

            for (j = 0; j < array.length; j++)
                if (i & 2n ** to_integer(j))
    ...
    var orderings = (data) => {
        ...
        for (var a, n, f, j, i = 0n; i < factorial([to_integer(array.length)]); i++) {
            a = array;
            n = i;
            ...
            for (j = 0; j < array.length; j++) {
                f = factorial([to_integer(array.length - j - 1)]);
    ...
    var combinations_n = (data, number) => {
        ...
        for (var j, k = -1, i = 0n; i < 2n ** to_integer(array.length); i++) {
            if (parity([i]) != number)
                continue;
    ...
    var group = (data, amount) => {
        ...
        for (var g = -1, i = 0; i < array.length; i++) {
            if (groups.length == 0 || groups[g].length == number) {
                groups.push([]);
                g++;
            }
    ...
    var max_compare = (...data) => -compare(...data);

    var min = (...data) => compare(...data) != 1n ? data[0] : data[1];
    var max = (...data) => compare(...data) == 1n ? data[0] : data[1];

    var sort_with = (data, comparator) => {
        var combine = (...arrays) => {
            var combined = [];

            while (arrays[0].length != 0 || arrays[1].length != 0) {
                if (arrays[0].length == 0) {
                    combined.push(arrays[1].shift());

                    continue;
                }

                if (arrays[1].length == 0) {
                    combined.push(arrays[0].shift());

                    continue;
                }

                combined.push(comparator(arrays[0][0], arrays[1][0]) == 1n ? arrays[1].shift() : arrays[0].shift());
            }
        ...
        var sort = (array) => {
            ...
            if (array.length == 2)
                return comparator(array[0], array[1]) != 1n ? [array[0], array[1]] : [array[1], array[0]];

            return combine(...partition_half(array).map(sort));
        };

        return sort(to_array(data));
    };

    var array_min = (data) => sort_with(data, compare)[0];
    var array_max = (data) => sort_with(data, max_compare)[0];

    var sum_code_points = (data) => sum(code_point(data));
    var code_point_range = (data) => sort_with(unique(code_point(data)), compare);

    var distance = (...data) => {
        var arrays = data.map(d => to_float_or_integer(to_array(d)));
        var max = array_max(arrays.map(a => a.length));

        var numbers = arrays.map(a => [...a, ...multiply_all([0n], max)].slice(0, max));

        if (arrays.some(a => is_float(a[0]))) {
            numbers = numbers.map(n => n.map(to_float));

            return root(sum(range(0, max).map(i => subtract(...numbers.map(n => n[i])) ** 2)), 2);
        } else {
            numbers = numbers.map(a => a.map(to_integer));

            return root(sum(range(0, max).map(i => subtract(...numbers.map(n => n[i])) ** 2n)), 2n);
        }
    };
    ...
    var remove = (data, position) => {
        ...
        position = sort_with(to_array(position).map(p => to_float(to_integer(p))), max_compare);

        for (var i = 0; i < position.length; i++)
            array = concat(array.slice(0, position[i]), array.slice(position[i] + 1));
    ...
    var array_range = (data) => {
        ...
        var sorted = sort_with(numbers, compare);
    ...
    var regular = (data) => {
        ...
        return regular_0 && data.every(d => d.length == data[0].length && identical(regular(d), regular_0)) ? [to_integer(data.length), ...regular_0] : 0n;
    };

    var depth = (data) => {
        var depth = regular(data);

        return depth ? (data.length ? to_integer(depth.length) : 0n) : -1n;
    };
    ...
    var filter_multidimensional = (arrays, dimensions) => {
        ...
        var points = arrays.map(a => pad(take_length(to_array(a).map(d => max(to_integer(d), 0n)), dimensions), dimensions.length, 0n));
    ...
    var to_bytes = (number, bytes) => {
        ...
        if (number >= 0)
            return pad(to_base(number, 16), -Number(bytes) * 2, "0");

        return pad(to_base(bitwise_not((positive(number) - 1n) % (2n ** (bytes * 8n)), 8n * bytes), 16), -Number(bytes) * 2, "0");
    };
    ...
    var to_json = (data) => {
        ...
        if (is_string(data))
            return "\"" + data.split("").map(c => {
                ...
                if (code_point(c) < 0x20)
                    return "\\u" + to_bytes(code_point(c), 2);
                
                return c;
            }).join("") + "\"";
        
        return "[" + data.map(to_json).join(",") + "]";
    };
    ...
    var format_program = (data) => {
        if (typeof data == "string")
            ...
            return flat(data.map(format_program));
        else
    ...
    var format_input = (data) => {
        if (typeof data == "number" || typeof data == "bigint" || typeof data == "string")
            ...
            return data.map(format_input).filter(d => d !== null);
        else
            throw "ERR: Invalid input format";
    };
    
    inputs = format_input(inputs).filter(d => d !== null);
    ...
    var string_token = (slice) => {
        ...
        if (point >= 0x0b && point <= 0x0d)
            return [...slice, code_page[0x00]].slice(0, 2);
        if (point == 0x0e)
            return [...slice, code_page[0x00], code_page[0x00]].slice(0, 3);
        if (point == 0x0f)
            return [...slice, code_page[0x00], code_page[0x00], code_page[0x00], code_page[0x00]].slice(0, 5);
        if (point >= 0x80 && point <= 0x9f || point >= 0xa0 && point <= 0xbf && point % 0x10 <= 0x03)
            return [...slice, code_page[0x00], code_page[0x00]].slice(0, 2 + (point % 0x10 >= 0x06));
        if (point >= 0xf0 && point <= 0xf2 || point >= 0xf6 && point <= 0xf8)
            return slice[0];
        if (point >= 0xf3 && point <= 0xf5 || point == 0xfa)
            return [slice[0], ...string_token(slice.slice(1))];
        if (point == 0xf9)
            return [...slice, code_page[0x00]].slice(0, 2);
        if (point == 0xfb)
            return [...[...slice, code_page[0x00]].slice(0, 2), ...string_token(slice.slice(2))];
        
        if (point == 0xfc) {
            var tokens = [slice[0]];
            var offset = 1;
            
            while (tokens[tokens.length - 1][0] != code_page[0xfd] && offset < slice.length) {
    ...
    var string_arguments = (string) => {
        ...
        var non_multi_string_token = (string) => {
            var point = code_page.indexOf(string[0]);
            ...
            if (point >= 0x80 && point <= 0x9f || point >= 0xa0 && point <= 0xbf && point % 0x10 <= 0x03)
                return string.slice(0, 2 + (point % 0x10 >= 0x06));
            if (point == 0xf9)
                return string.slice(0, 2);
            if (point == 0xfb)
                return string.slice(0, 2);
        ...
        while (position < string.length) {
            token = non_multi_string_token(string.slice(position));
    ...
    var parse = (tokens) => {
        ...
        var read_at_depth = (arg_depth = depth) => {
            var j, t = tree;
        ...
        var count_default = (data) => {
            var count = (data) => {
                if (data.type == "operator" && data.name != "α" && data.name != "β" && data.name != "τ")
                    return data.data.reduce((a, d) => a + count(d), 0) + (arities[data.name] - data.data.length);
                if (data.type == "operator")
                    return data.data.length >= [1, 1, 2][["α", "β", "τ"].indexOf(data.name)] ? (data.arity + [1, 1, 2][["α", "β", "τ"].indexOf(data.name)]) - data.data.length : 0;
                if (data.type == "string")
                    return string_arguments(data.string) - data.data.length;

                return 0;
            }
            
            return data.reduce((a, d) => a + count(d), 0);
        };
        
        var determine_arity = (data) => {
            if (data.type == "operator")
                return data.name == "α" || data.name == "β" ? data.arity : arities[data.name];
            if (data.type == "function")
                return Math.min(count_default(data.data), 4);
            if (data.type == "string")
                return string_arguments(data.string);
        ...
        for (i = 0; i < tokens.length; i++) {
            ...
            switch (p) {
                ...
                case "}":
                    ...
                    if (needed.length && item(needed, -1).type == "modifier" && !item(needed, -1).number) {
                        while (needed.length && item(needed, -1).type == "modifier" && !item(needed, -1).number) {
                            arity = array_max(read_at_depth().data.map(determine_arity));
                            
                            read_at_depth().arity = arity;
                            read_at_depth().is_dynamic = read_at_depth().data.some(d => dynamic_operators.includes(d.name));
                    ...
                    if (arity != 0) {
                        ...
                        while (needed.length && !item(needed, -1).number) {
                            needed.pop();
                ...
                case "“":
                    ...
                    while (needed.length && !needed[needed.length - 1].number) {
                        needed.pop();
                ...
                case "”":
                    ...
                    if (needed.length && item(needed, -1).type == "modifier" && !item(needed, -1).number) {
                        ...
                        arity = string_arguments(tokens[i]);
                    }
                ...
                default:
                    ...
                    if (needed.length && item(needed, -1).type == "modifier" && !item(needed, -1).number) {
                        while (needed.length && item(needed, -1).type == "modifier" && !item(needed, -1).number) {
                            arity = array_max(read_at_depth().data.map(determine_arity));
                            is_dynamic = is_dynamic || read_at_depth().data.some(d => dynamic_operators.includes(d.name));
                            
                            read_at_depth().arity = arity;
                            read_at_depth().is_dynamic = is_dynamic;
                        ...
                        arity = arities[tokens[i].join("")];
                        is_dynamic = dynamic_operators.includes(tokens[i].join(""));
                    }
        ...
        while (1) {
            ...
            if (depth > pointer.length || (data.type == "operator" || data.type == "string") && !data.data.length) {
                if (data.type == "operator" || data.type == "string") {
                    var arg_count = data.type == "operator" ? arities[data.name] : string_arguments(data.string);
                    
                    if (data.name != "α" && data.name != "β" && data.name != "τ") {
                        var parent = read_at_pointer(pointer.slice(0, -1));
                        
                        while (data.data.length < arg_count)
                            data.data.push({
                                type: "default"
                            });
                    } else {
                        while (data.data.length < data.arity + [1, 1, 2][["α", "β", "τ"].indexOf(data.name)])
                            data.data.push({
                                type: "default"
                            });
                    }
    ...
    var string_notation = (string, data) => {
        ...
        var notation_substring = (string, data) => {
            ...
            while (output.width < string.length) {
                ...
                if (string[output.width - s.width] == code_page[0xfd])
                    break;
            }
        ...
        switch (code_page.indexOf(string[0])) {
            ...
            case 0x09:
                return {
                    width: 1,
                    string: ["hello", "world", "Hello", "World", "Hello, world!", "Hello, World!", "Hello world", "Hello world!", "Hello, world", "Hello, "][code_page.indexOf(string[0])]
                };
            case 0x0b:
                return {
                    width: 2,
            ...
            case 0x1f:
                return {
                    width: 1,
                    string: [
                       "Fizz", "Buzz", "FizzBuzz", "bottles of beer on the wall", "bottles of beer", "ake one down, pass it around",
                       "Code Golf and Coding Challenges", "Programming Puzzles and Code Golf", "Code Golf",
                       "code", "golf", "Code", "Golf", "Ash", "ash", "FrickBrick"
                    ][code_page.indexOf(string[0]) - 0x10]
                };
            case 0xa4:
            case 0xa5:
            case 0xa6:
            ...
            case 0xaf:
                return {
                    width: 1,
                    string: [", ", ". ", ": ", "...", "    ", "],[", "{\"", "\":", "},", "\n\n", "'t", "'s"][code_page.indexOf(string[0]) - 0xa4]
                };
            case 0xb4:
            case 0xb5:
            case 0xb6:
            ...
            case 0xbf:
                return {
                    width: 1,
                    string: ["The ", "the ", "is ", "a ", "s ", "Th", "to ", "ing ", "‘", "’", "“", "”"][code_page.indexOf(string[0]) - 0xb4]
                };
            case 0xc0:
            case 0xc1:
            case 0xc2:
            ...
            case 0xcf:
                return {
                    ...
                    string: [
                        ...
                        "eight", "nine", "ten", "eleven", "twelve", "thir", "fif", "eigh"
                    ][code_page.indexOf(string[0]) - 0xc0]
                };
            case 0xd0:
            case 0xd1:
            case 0xd2:
            ...
            case 0xdf:
                return {
                    width: 1,
                    string:[
                        "teen", "ty", "twenty", "hundred", "thousand", "million", "billion", "trillion",
                        "null", "true", "false", "[]", "", "", "", ""
                    ][code_page.indexOf(string[0]) - 0xd0]
                };
            case 0xe0:
            case 0xe1:
            case 0xe2:
            ...
            case 0xef:
                return {
                    ...
                    string: [
                        "", "", "", "", "", "", "", "",
                        "", "", "", "", "", "", "", ""
                    ][code_page.indexOf(string[0]) - 0xe0]
                };
            case 0xf0:
            case 0xf1:
            case 0xf2:
            ...
            case 0xfa:
                ...
                return {
                    width: 1 + register.width,
                    string: upper_case(register.string)
                };
            case 0xfb:
                var register = string_notation(string.slice(2), data);
                
                return {
                    width: 2 + register.width,
                    string: multiply_all(register.string, code_page.indexOf(string[1]) + 1)
                };
            case 0xfc:
    ...
    var print_data = [];
    var print = (data) => (print_data.push(data), data);
    
    var all_arguments = [];
    var default_arguments = [];
    
    var interpret_data = (data, arguments = []) => {
        if (data.type == "operator") {
            switch (data.name) {
                case "↑":
                    return overload(data.data.map(d => interpret_data(d)), float_power, float_power, remove, remove);
                case "↓":
                    return overload(data.data.map(d => interpret_data(d)), rational_power, rational_power, insert, insert);
                case "←":
                    return overload(data.data.map(d => interpret_data(d)), logical_not_and, logical_not_and, prepend, prepend);
                case "→":
                    return overload(data.data.map(d => interpret_data(d)), logical_and_not, logical_and_not, append, append);
                case "⟨":
                    return min(...data.data.map(d => interpret_data(d)));
                case "⟩":
                    return max(...data.data.map(d => interpret_data(d)));
                case "⌈":
                    return overload(data.data.map(d => interpret_data(d)), float_pass(Math.ceil), float_pass(Math.ceil), pass(item, [], [-1]), pass(item, [], [-1]));
                case "⌉":
                    return overload(data.data.map(d => interpret_data(d)), float_pass(Math.round), float_pass(Math.round), pass(slice_to, [], [-1]), pass(slice_to, [], [-1]));
                case "⌊":
                    return overload(data.data.map(d => interpret_data(d)), float_pass(Math.floor), float_pass(Math.floor), pass(item, [], [0]), pass(item, [], [0]));
                case "⌋":
                    return overload(data.data.map(d => interpret_data(d)), float_pass(Math.trunc), float_pass(Math.trunc), pass(slice_from, [], [1]), pass(slice_from, [], [1]));
                case "↕":
                    return overload(data.data.map(d => interpret_data(d)), group_digits, group_digits, group, group);
                case "↔":
                    return overload(data.data.map(d => interpret_data(d)), i_part, i_part, is_safe_number, flat);
                case "⌐":
                    return overload(data.data.map(d => interpret_data(d)), pass(float_divide, [1]), pass(float_divide, [1]), reverse, reverse);
                case "¬":
                ...
                case "α":
                    return (() => {
                        ...
                        if (data.is_dynamic) {
                            var inputs = data.data.slice(2).map(d => interpret_data(d));
                            
                            var with_f = {...data.data[0]};
                            
                            with_f.data = with_f.data[0].type == "default" ? [data.data[1], ...with_f.data.slice(1)] : [with_f.data[0], data.data[1], ...with_f.data.slice(2)];
                            
                            if (!inputs.some(i => is_array(i)))
                                return interpret_data({
                                    type: "function",
                                    data: [with_f]
                                }, inputs);
                            
                            var max = array_max(inputs.filter(is_array).map(length));
                        
                            inputs = inputs.map(i => is_array(i) ? i : multiply_all([i], max));
                            
                            return range(0, max).map(i => inputs.map(n => i in n ? n[i] : null)).map(i => {
                                var defaults = reverse(flat(reverse(all_arguments)));

                                return interpret_data({
                                    type: "function",
                                    data: [with_f]
                                }, i.map(n => n !== null ? n : defaults.length ? defaults.pop() : 0n));
                            });
                        }
                        
                        var inputs = data.data.slice(1).map(d => interpret_data(d));
                ...
                case "∂":
                    return overload(data.data.map(d => interpret_data(d)), f_part, f_part, (string) => to_string(sort_with(string, compare)), (array) => sort_with(array, compare));
                case "∫":
                    return overload(data.data.map(d => interpret_data(d)), unimplemented, unimplemented, is_alphanum, cumulative_sum);
                case "Δ":
                    return distance(...data.data.map(d => interpret_data(d)));
                case "δ":
                    return overload(data.data.map(d => interpret_data(d)), to_alpha, to_alpha, is_alpha, array_delta);
                case "≡":
                    return to_boolean(interpret_data(data.data[0]));
                case "≠":
                    return 1n - identical(...data.data.map(d => interpret_data(d)));
                case "≤":
                    return BigInt(compare(...data.data.map(d => interpret_data(d))) <= 0n);
                case "≥":
                    return BigInt(compare(...data.data.map(d => interpret_data(d))) >= 0n);
                case "÷":
                    return integer_divide(...data.data.map(d => interpret_data(d)));
                case "±":
                    return compare(...data.data.map(d => interpret_data(d)));
                case " ":
                    return 0n;
                case "!":
                    return overload(data.data.map(d => interpret_data(d)), factorial, factorial, array_factorial, array_factorial);
                case "\"":
                    return "";
                case "%":
                    return modulo(...data.data.map(d => interpret_data(d)));
                case "&":
                    return logical_and(...data.data.map(d => interpret_data(d)));
                case "(":
                    return overload(data.data.map(d => interpret_data(d)), range, range, alpha_compare, read_dictionary);
                case ")":
                    return overload(data.data.map(d => interpret_data(d)), unimplemented, unimplemented, overwrite, write_dictionary);
                case "*":
                    return multiply(...data.data.map(d => interpret_data(d)));
                case "+":
                    return add(...data.data.map(d => interpret_data(d)));
                case ",":
                    return [...data.data.map(d => interpret_data(d))];
                case "-":
                    return subtract(...data.data.map(d => interpret_data(d)));
                case ".":
                    return [...data.data.map(d => interpret_data(d))];
                case "/":
                    return float_divide(...data.data.map(d => interpret_data(d)));
                case "0":
                case "1":
                case "2":
                case "3":
                ...
                case "9":
                    return +data.name;
                case ":":
                    return string_concat(...data.data.map(d => interpret_data(d)));
                case ";":
                    return array_concat(...data.data.map(d => interpret_data(d)));
                case "<":
                    return BigInt(compare(...data.data.map(d => interpret_data(d))) == -1n);
                case "=":
                    return identical(...data.data.map(d => interpret_data(d)));
                case ">":
                    return BigInt(compare(...data.data.map(d => interpret_data(d))) == 1n);
                case "?":
                    return to_boolean(interpret_data(data.data[0])) ? interpret_data(data.data[1]) : interpret_data(data.data[2]);
                case "A":
                    return overload(data.data.map(d => interpret_data(d)), all_factors, all_factors, permutations, permutations);
                case "B":
                    return base_convert(...data.data.map(d => interpret_data(d)));
                case "C":
                    return overload(data.data.map(d => interpret_data(d)), bitwise_or, bitwise_or, combinations_n, combinations_n);
                case "D":
                    return to_number_base(...data.data.map(d => interpret_data(d)));
                case "E":
                    return base_convert(interpret_data(data.data[0]), 16);
                case "F":
                    return overload(data.data.map(d => interpret_data(d)), pass(multiply_power, [], [10n]), pass(multiply_power, [], [10]), slice_from, slice_from);
                case "G":
                    return overload(data.data.map(d => interpret_data(d)), decimal_string, decimal_string, pass(group, [], [2]), pass(group, [], [2]));
                case "H":
                    return overload(data.data.map(d => interpret_data(d)), bitwise_and, bitwise_and, permutations_n, permutations_n);
                case "I":
                    return overload(data.data.map(d => interpret_data(d)), is_prime, is_prime, all_identical, all_identical);
                case "J":
                    return overload(data.data.map(d => interpret_data(d)), fixed_point, fixed_point, index_of, index_of);
                case "K":
                    return overload(data.data.map(d => interpret_data(d)), pass(divide_power, [], [10n]), pass(divide_power, [], [10]), slice_to, slice_to);
                case "L":
                    return overload(data.data.map(d => interpret_data(d)), pass(multiply_power, [], [2n]), pass(multiply_power, [], [2]), rotate_left, rotate_left);
                case "M":
                    return overload(data.data.map(d => interpret_data(d)), pass(multiply, [10n]), pass(multiply, [10]), array_min, array_min);
                case "N":
                    return base_convert(interpret_data(data.data[0]), 2);
                case "O":
                    return overload(data.data.map(d => interpret_data(d)), prime_factors, prime_factors, orderings, orderings);
                case "P":
                    return overload(data.data.map(d => interpret_data(d)), nth_prime, nth_prime, all_slices, all_slices);
                case "Q":
                    return overload(data.data.map(d => interpret_data(d)), root, root, multiply_items, multiply_items);
                case "R":
                    return overload(data.data.map(d => interpret_data(d)), pass(divide_power, [], [2n]), pass(divide_power, [], [2]), rotate_right, rotate_right);
                case "S":
                    return overload(data.data.map(d => interpret_data(d)), is_between, is_between, slice_between, slice_between);
                case "T":
                    return base_convert(interpret_data(data.data[0]), 3);
                case "U":
                    return overload(data.data.map(d => interpret_data(d)), bitwise_xor, bitwise_xor, shift_alpha, fill);
                case "V":
                    return overload(data.data.map(d => interpret_data(d)), unimplemented, unimplemented, includes, includes);
                case "W":
                    return overload(data.data.map(d => interpret_data(d)), logarithm, logarithm, interleave, interleave);
                case "X":
                    return overload(data.data.map(d => interpret_data(d)), pass(multiply, [16n]), pass(multiply, [16]), array_max, array_max);
                case "Y":
                    return overload(data.data.map(d => interpret_data(d)), pass(range_inclusive, [1n]), pass(range_inclusive, [1]), overlapping_pairs, overlapping_pairs);
                case "Z":
                    return overload(data.data.map(d => interpret_data(d)), pass(range, [0n]), pass(range, [0]), count_all_items, count_all_items);
                case "[":
                    return overload(data.data.map(d => interpret_data(d)), digit, digit, item, item);
                case "]":
                    return overload(data.data.map(d => interpret_data(d)), write_digit, write_digit, write, write);
                case "^":
                    return overload(data.data.map(d => interpret_data(d)), power, power, multiply_all, multiply_all);
                case "_":
                    return overload(data.data.map(d => interpret_data(d)), pass(multiply, [-1n]), pass(multiply, [-1]), invert_case, (array) => to_float_or_integer(array).map(n => -n));
                case "a":
                    return to_array(interpret_data(data.data[0]));
                case "b":
                    return overload(data.data.map(d => interpret_data(d)), parity, parity, duplicates, duplicates);
                case "c":
                    return overload(data.data.map(d => interpret_data(d)), factors, factors, combinations, combinations);
                case "d":
                    return overload(data.data.map(d => interpret_data(d)), pass(multiply, [], [2n]), pass(multiply, [], [2]), pass(multiply_all, [], [2]), pass(multiply_all, [], [2]));
                case "e":
                    return overload(data.data.map(d => interpret_data(d)), unimplemented, unimplemented, unimplemented, unimplemented);
                case "f":
                    return to_float(interpret_data(data.data[0]));
                case "g":
                    return overload(data.data.map(d => interpret_data(d)), negative, negative, lower_case, (data) => sort_with(data, compare));
                case "h":
                    return overload(data.data.map(d => interpret_data(d)), pass(integer_divide, [], [2n]), pass(float_divide, [], [2]), partition_half, partition_half);
                case "i":
                    return to_integer(interpret_data(data.data[0]));
                case "j":
                    return overload(data.data.map(d => interpret_data(d)), pass(logarithm, [], [2n]), pass(logarithm, [], [2]), mirror_horizontal, average);
                case "k":
                    return overload(data.data.map(d => interpret_data(d)), take_sign, take_sign, take_case, determine_order);
                case "l":
                    return length(interpret_data(data.data[0]));
                case "m":
                    return overload(data.data.map(d => interpret_data(d)), pass(add, [-1n]), pass(add, [-1]), pass(slice_between, [], [1, -1]), pass(slice_between, [], [1, -1]));
                case "n":
                    return to_number(interpret_data(data.data[0]));
                case "o":
                    return overload(data.data.map(d => interpret_data(d)), pass(add, [1n]), pass(add, [1]), duplicate_first, duplicate_first);
                case "p":
                    return overload(data.data.map(d => interpret_data(d)), positive, positive, upper_case, (data) => sort_with(data, max_compare));
                case "q":
                    return overload(data.data.map(d => interpret_data(d)), pass(root, [], [2n]), pass(root, [], [2]), trim, product);
                case "r":
                    return overload(data.data.map(d => interpret_data(d)), round_to_n, round_to_n, float_from_sig_figs, float_from_sig_figs);
                case "s":
                    return to_string(interpret_data(data.data[0]));
                case "t":
                    return overload(data.data.map(d => interpret_data(d)), pass(multiply, [], [3n]), pass(multiply, [], [3]), pass(multiply_all, [], [3]), pass(multiply_all, [], [3]));
                case "u":
                    return overload(data.data.map(d => interpret_data(d)), pass(power, [], [2n]), pass(power, [], [2]), sum_code_points, sum);
                case "v":
                    return overload(data.data.map(d => interpret_data(d)), unsigned_addition, unsigned_addition, partition, partition);
                case "w":
                    return overload(data.data.map(d => interpret_data(d)), sign, sign, determine_case, is_sorted);
                case "x":
                    return overload(data.data.map(d => interpret_data(d)), pass(logarithm, [], [10n]), pass(logarithm, [], [10]), modes, modes);
                case "y":
                    return overload(data.data.map(d => interpret_data(d)), pass(modulo, [], [2n]), pass(modulo, [], [2]), unique, unique);
                case "z":
                    return overload(data.data.map(d => interpret_data(d)), n_parts, n_parts, split, join);
                case "|":
                    return logical_or(...data.data.map(d => interpret_data(d)));
                case "~":
                    return overload(data.data.map(d => interpret_data(d)), bitwise_not, bitwise_not, rotate_clockwise, rotate_clockwise);
                case "·":
                    return overload(data.data.map(d => interpret_data(d)), pass(range_inclusive, [0n]), pass(range_inclusive, [0]), unimplemented, array_range);
                case "Ȧ":
                    return overload(data.data.map(d => interpret_data(d)), count_digits, count_digits, is_num, neighbors);
                case "Ḃ":
                    return overload(data.data.map(d => interpret_data(d)), pass(divisible, [], [2n]), pass(divisible, [], [2]), pass(divisible, [], [2]), pass(length_divisible, [], [2]));
                case "Ċ":
                    return overload(data.data.map(d => interpret_data(d)), pass(divisible, [], [3n]), pass(divisible, [], [3]), pass(divisible, [], [3]), (array) => array.length == 1 ? 1n : 0n);
                case "Ḋ":
                    return overload(data.data.map(d => interpret_data(d)), pass(divisible, [], [10n]), pass(divisible, [], [10]), pass(divisible, [], [10]), pass(length_divisible, [], [10]));
                case "Ė":
                    return overload(data.data.map(d => interpret_data(d)), unimplemented, unimplemented, insert_merge, insert_merge);
                case "Ḟ":
                    return overload(data.data.map(d => interpret_data(d)), nth_fibonacci, nth_fibonacci, title_case, array_fibonacci);
                case "Ġ":
                    return overload(data.data.map(d => interpret_data(d)), greatest_common_divisor, greatest_common_divisor, intersection, intersection);
                case "Ḣ":
                    return overload(data.data.map(d => interpret_data(d)), unimplemented, unimplemented, to_2d_array, transpose);
                case "İ":
                    return overload(data.data.map(d => interpret_data(d)), sig_figs, sig_figs, integer_from_sig_figs, integer_from_sig_figs);
                case "Ŀ":
                    return overload(data.data.map(d => interpret_data(d)), least_common_multiple, least_common_multiple, union, union);
                case "Ṁ":
                    return overload(data.data.map(d => interpret_data(d)), nth_fibonacci, nth_fibonacci, replace_map, replace_map);
                case "Ṅ":
                    return overload(data.data.map(d => interpret_data(d)), is_negative, is_negative, is_lower_case, is_min_sorted);
                case "Ȯ":
                    return overload(data.data.map(d => interpret_data(d)), pass(divisible, [], [8n]), pass(divisible, [], [8]), pass(divisible, [], [8]), pass(length_divisible, [], [8]));
                case "Ṗ":
                    return overload(data.data.map(d => interpret_data(d)), is_positive, is_positive, is_upper_case, is_max_sorted);
                case "Ṙ":
                    return overload(data.data.map(d => interpret_data(d)), pass(divisible, [], [5n]), pass(divisible, [], [5]), pass(divisible, [], [5]), (array) => array.length == 2 ? 1n : 0n);
                case "Ṡ":
                    return overload(data.data.map(d => interpret_data(d)), unimplemented, unimplemented, pad, pad);
                case "Ṫ":
                    return overload(data.data.map(d => interpret_data(d)), range_inclusive, range_inclusive, take_length, take_length);
                case "Ẇ":
                    return overload(data.data.map(d => interpret_data(d)), divmod, divmod, difference, difference);
                case "Ẋ":
                    return overload(data.data.map(d => interpret_data(d)), logical_nor, logical_nor, shift_top, shift_top);
                case "Ẏ":
                    return overload(data.data.map(d => interpret_data(d)), logical_nand, logical_nand, unimplemented, unimplemented);
                case "Ż":
                    return overload(data.data.map(d => interpret_data(d)), base_256, base_256, map_to_pairs, map_to_pairs);
                case "ȧ":
                    return is_array(interpret_data(data.data[0]));
                case "ḃ":
                    return (() => {
                        var bit_array = pad(to_array(interpret_data(data.data[0])), 2, 0n).map(to_boolean);
                        
                        if (bit_array[0] == 0n && bit_array[1] == 0n)
                            return [];
                        if (bit_array[0] != bit_array[1])
                            return [interpret_data(data.data[bit_array[1] + 1n])];
                        
                        return [interpret_data(data.data[1]), interpret_data(data.data[2]), interpret_data(data.data[3])];
                    })();
                case "ċ":
                    return (() => {
                        var bit_array = pad(to_array(interpret_data(data.data[0])), 2, 0n).map(to_boolean);
                        ...
                        if (bit_array[0] == 0n && bit_array[1] == 0n)
                            return [interpret_data(data.data[3])];
                        if (bit_array[0] != bit_array[1])
                            return [interpret_data(data.data[bit_array[1] + 1n])];
                        
                        return [interpret_data(data.data[1]), interpret_data(data.data[2])];
                    })();
                case "ḋ":
                    return (() => {
                        var bit_array = pad(to_array(interpret_data(data.data[0])), 2, 0n).map(to_boolean);
                        ...
                        if (bit_array[0] == 0n && bit_array[1] == 0n)
                            return interpret_data(data.data[1]);
                        if (bit_array[0] != bit_array[1])
                            return interpret_data(data.data[2]);
                        
                        return interpret_data(data.data[3]);
                    })();
                case "ė":
                    return to_json(interpret_data(data.data[0]));
                case "ḟ":
                    return is_float(interpret_data(data.data[0]));
                case "ġ":
                    return overload(data.data.map(d => interpret_data(d)), pass(power, [10n]), pass(power, [10]), pass(split, [], ["\n"]), pass(join, [], ["\n"]));
                case "ḣ":
                    return overload(data.data.map(d => interpret_data(d)), random_to_n, random_to_n, random_item, random_item);
                case "ṅ":
                    return is_number(interpret_data(data.data[0]));
                case "ȯ":
                    return overload(data.data.map(d => interpret_data(d)), to_sign, to_sign, is_whitespace, default_filter);
                case "ṗ":
                    return print(interpret_data(data.data[0]));
                case "ṙ":
                    return Math.random();
                case "ṡ":
                ...
                case "ẇ":
                    return overload(data.data.map(d => interpret_data(d)), pass(power, [2n]), pass(power, [2]), pass(split, [], [" "]), pass(join, [], [" "]));
                case "ż":
                    return is_integer(interpret_data(data.data[0]));
                case "Ạ":
                    return (() => {
                        var input_data = interpret_data(data.data[1]);
                        
                        if (is_number(input_data)) {
                            if (to_boolean(input_data))
                                return interpret_data(data.data[0], [input_data]);
                            
                            return 0n;
                        } else {
                            var array = to_array(input_data);

                            for (var i = 0; i < array.length; i++)
                                if (!to_boolean(interpret_data(data.data[0], [array[i], i, input_data])))
                                    return 0n;
                    ...
                    return (() => {
                        ...
                        if (is_number(input_data)) {
                            ...
                            for (var i = 0; i < array.length; i++) {
                                ...
                                if (interpret_data(data.data[0], [array[i], i, take_type(partition, input_data), input_data])) {
                            ...
                            return partitions.map(p => take_type(p, input_data));
                        }
                    })();
                case "Ḍ":
                    return (() => {
                        ...
                        if (is_number(input_data)) {
                            var range_max = to_integer(Math.max(Math.ceil(to_float(input_data)), 0));
                            
                            var final;
                            
                            for (var i = 0n; i < range_max; i++)
                                final = interpret_data(data.data[0], [i, range_max]);
                            
                            return final;
                        } else {
                            var array = to_array(input_data).filter(d => is_array(d) && d.length >= 2);

                            var keys = unique(array.map(d => d[0]));
                            var dictionary = keys.map(k => array.find(a => identical(a[0], k)).slice(0, 2));

                            var result = [];

                            for (var i = 0; i < dictionary.length; i++)
                                result.push([dictionary[i][0], interpret_data(data.data[0], [dictionary[i][1], dictionary[i][0], dictionary, array.filter(a => identical(a[0], dictionary[i][0])).map(a => a[1])])]);
                ...
                case "Ḥ":
                    return (() => {
                        ...
                        if (is_number(input_data)) {
                            var range_max = to_integer(Math.max(Math.floor(to_float(input_data)), 0));
                            
                            var array = [];
                            
                            for (var i = 1n; i <= range_max; i++)
                                array.push(interpret_data(data.data[0], [i, range_max]));
                ...
                case "Ḳ":
                    return (() => {
                        ...
                        if (is_number(input_data)) {
                            var previous = input_data;
                            var count = 0n;
                            
                            while (to_boolean(previous)) {
                                previous = interpret_data(data.data[0], [previous, input_data]);
                            ...
                            for (var i = 0; i < array.length; i++)
                                result.push(interpret_data(data.data[0], [array[i], i, input_data, take_type([...result], input_data)]));

                            return take_type(result, input_data);
                        }
                    })();
                case "Ḷ":
                    return (() => {
                        ...
                        if (is_number(input_data)) {
                            ...
                            for (var i = 0n; i < range_max; i++)
                                if (to_boolean(interpret_data(data.data[0], [i, range_max])))
                                    array.push(i);
                            ...
                            for (var i = 0; i < array.length; i++)
                                if (to_boolean(interpret_data(data.data[0], [array[i], i, input_data, take_type([...result], input_data)])))
                                    result.push(array[i]);
                ...
                case "Ṃ":
                    return (() => {
                        ...
                        if (is_number(input_data)) {
                            var range_max = to_integer(Math.max(Math.floor(to_float(input_data)), -1));
                            ...
                            var multidimensional_map = (array, position) => array.map((d, i) => is_array(d) ? multidimensional_map(d, [...position, i]) : interpret_data(data.data[0], [d, [...position, i], input_data]));

                            return multidimensional_map(to_array(input_data), []);
                        }
                    })();
                case "Ṇ":
                    return (() => {
                ...
                case "Ọ":
                    return (() => {
                        ...
                        while (to_boolean(previous))
                            previous = interpret_data(data.data[0], [previous]);
                ...
                case "Ṛ":
                    return (() => {
                        ...
                        if (is_number(input_data)) {
                            ...
                            for (var i = 1; i < array.length; i++)
                                previous = interpret_data(data.data[0], [previous, array[i], i, input_data]);
                ...
                case "Ṭ":
                    return (() => {
                        ...
                        if (is_number(input_data)) {
                            ...
                            while (!identical(previous, input_data))
                                previous = interpret_data(data.data[0], [previous, input_data]);
                            
                            return previous;
                        } else {
                            return take_type(sort_with(to_array(input_data), (...items) => sign(to_integer(interpret_data(data.data[0], [items[0], items[1], input_data])))), input_data);
                        }
                    })();
                case "Ụ":
                    return (() => {
                        ...
                        if (is_number(input_data)) {
                            var previous = 1n;
                            var counter = to_integer(input_data);
                            
                            while (to_boolean(previous)) {
                                previous = interpret_data(data.data[0], [counter, to_integer(input_data)]);
                            ...
                            for (var i = 0; i < array.length; i++)
                                sort.push(interpret_data(data.data[0], [array[i], i, input_data, take_type([...sort], input_data)]));

                            var positions = range(0, array.length);

                            positions = sort_with(positions, (...items) => compare(sort[items[0]], sort[items[1]]));

                            return take_type(map_other_item(positions, array), input_data);
                        }
                    })();
                case "Ṿ":
                    return (() => {
                        ...
                        if (is_number(input_data)) {
                            ...
                            positions = sort_with(positions, (...items) => max_compare(sort[items[0]], sort[items[1]]));
                ...
                case "Ẉ":
                    return (() => {
                        var input_data = data.data.slice(1).map(d => interpret_data(d));
                        
                        if (is_number(input_data[0])) {
                            var i_range = range_inclusive(...input_data);
                            
                            var array = [];
                            
                            for (var i = 0; i < i_range.length; i++)
                                array.push(interpret_data(data.data[0], [i_range[i], i_range[0], i_range[i_range.length - 1], input_data[1]]));
                            
                            return array;
                        } else {
                            var arrays = input_data.map(to_array);
                            var result = arrays[0].length >= arrays[1].length ? arrays[0] : arrays[1];

                            for (var i = 0; i < min(arrays[0].length, arrays[1].length); i++)
                                result[i] = interpret_data(data.data[0], [arrays[0][i], arrays[1][i], i, [input_data[0], input_data[1], [...result]]]);
                ...
                case "Ỵ":
                    return (() => {
                        var input_data = interpret_data(data.data[1]);
                        var input_position = interpret_data(data.data[2]);
                        
                        if (is_number(input_position)) {
                            var i_range = range(input_position, input_data);
                            ...
                            var array = to_array(input_data);
                            var position = item(range(0, array.length), input_position);

                            return take_type(write(array, position, interpret_data(data.data[0], [array[position], position, input_data, input_position])), input_data);
                        }
                    })();
                case "Ẓ":
                    return (() => {
                        ...
                        if (is_number(input_data)) {
                            ...
                            for (var i = 0; i < array.length; i++)
                                result.push([array[i], interpret_data(data.data[0], [array[i], i, input_data, [...result]])]);
                ...
                case "↵":
                    return "\n";
                case "⁽":
                    return item(all_arguments[0], to_integer(interpret_data(data.data[0])));
                case "⁾":
                    return item(reverse(flat(reverse(all_arguments))), to_integer(interpret_data(data.data[0])));
                case "₀":
                case "₁":
                case "₂":
                case "₃":
                    return item(all_arguments[all_arguments.length - 1], code_page.indexOf(data.name) - 0xd2);
                case "₄":
                case "₅":
                case "₆":
                    return item(reverse(flat(reverse(all_arguments.slice(0, -1)))), code_page.indexOf(data.name) - 0xd6);
                case "₇":
                case "₈":
                case "₉":
                    return item(all_arguments[0], code_page.indexOf(data.name) - 0xd9);
                case "₊":
            ...
            default_arguments.push(reverse(flat(reverse(all_arguments))));
            
            for (var i = 0; i < data.data.length; i++)
                return_data = interpret_data(data.data[i]);
            
            all_arguments.pop();
            default_arguments.pop();
            
            return return_data;
        } else if (data.type == "default") {
            return item(default_arguments, -1).length ? item(default_arguments, -1).pop() : 0n;
        } else if (data.type == "string") {
            string_counter = 0;
            
            if (data.string[0] == "‘")
                return string_notation(data.string.slice(1), data.data.map(d => d.type == "default" ? "" : interpret_data(d))).string;
            if (data.string[0] == "’")
            ...
            if (data.string[0] == "”") {
                ...
                var string_arguments = data.data.map(d => d.type == "default" ? "" : interpret_data(d));
                
                while (position < data.string.length) {
                    substring = string_notation(data.string.slice(position), string_arguments);
    ...
    var implicit_return = interpret_data(parse(tokenize(program)), [...inputs]);
    
    return [...print_data, implicit_return];
};
