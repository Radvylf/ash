var interpret = (program = [], input = []) => {
    ...
    var positive = (number) => number < 0 ? -number : number;
    var negative = (number) => number > 0 ? -number : number;
    ...
    var fixed_point = (number, point) => {
        ...
        var rounded = round_to_n(numbers);
    ...
    var duplicate_first = (data) => is_array(data) ? concat([data[0]], data) : data[0] + data;
    ...
    var rotate_n_times = (data, amount) => {
        ...
        if (amount == 0)
            return data;
        if (amount == 1)
            return rotate_clockwise(data);
        if (amount == 2)
            return rotate_180(data);
        return rotate_clockwise(rotate_180(data));
    };
    ...
    var interpret_data = (data) => {
        if (data.type == "operator") {
            switch (data.name) {
                ...
                case "m":
                    return overload(data.data.map(interpret_data), pass(add, [-1n]), pass(add, [-1]), pass(slice, [], [1, -1]), pass(slice, [], [1, -1]));
                case "n":
