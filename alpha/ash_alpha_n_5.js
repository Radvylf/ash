var interpret = (program = [], inputs = []) => {
            ...
            switch (p) {
                ...
                default:
                    ...
                    if (arities[tokens[i].join("")]) {
                        ...
                        if (dynamic_operators.includes(tokens[i].join(""))) {
                            insert_at_depth({
                                type: "function",
                                data: []
                            });
    ...
    var interpret_data = (data, arguments = []) => {
        var dynamic_wrapper = (dynamic_function, ...arguments) => {
            return dynamic_function(...arguments);
        };
        
        if (data.type == "operator") {
            switch (data.name) {
                case "↑":
                    return overload(data.data.map(d => interpret_data(d)), unimplemented, unimplemented, remove, remove);
                case "↓":
                    return overload(data.data.map(d => interpret_data(d)), unimplemented, unimplemented, insert, insert);
                case "←":
                ...
                case "∫":
                    return overload(data.data.map(d => interpret_data(d)), f_part, f_part, is_alphanum, cumulative_sum);
                case "Δ":
                    return overload(data.data.map(d => interpret_data(d)), distance, distance, distance, distance);
                case "∞":
                    return Infinity;
                case "≡":
                ...
                case "\"":
                    return overload(data.data.map(d => interpret_data(d)), logical_nor, logical_nor, index_of, index_of);
                case "#":
                    return overload(data.data.map(d => interpret_data(d)), unimplemented, unimplemented, is_whitespace, regular);
                case "$":
                    return overload(data.data.map(d => interpret_data(d)), bitwise_not, bitwise_not, rotate_n_times, rotate_n_times);
                case "%":
                    return modulo(...data.data.map(d => interpret_data(d)));
                case "&":
                    return logical_and(...data.data.map(d => interpret_data(d)));
                case "'":
                    return overload(data.data.map(d => interpret_data(d)), logical_nand, logical_nand, count, count);
                case "(":
                ...
                case ">":
                    return BigInt(compare(...data.data.map(d => interpret_data(d))) == 1n);
                case "@":
                    return overload(data.data.map(d => interpret_data(d)), to_sign, to_sign, group_square, depth);
                case "A":
                ...
                case "D":
                    return overload(data.data.map(d => interpret_data(d)), to_number_base, to_number_base, to_number_base, from_number_base);
                case "E":
                ...
                case "J":
                    return overload(data.data.map(d => interpret_data(d)), fixed_point, fixed_point, shift_top, shift_top);
                case "K":
                ...
                case "V":
                    return overload(data.data.map(d => interpret_data(d)), unimplemented, unimplemented, remove_slice, remove_slice);
                case "W":
                ...
                case "\\":
                    return overload(data.data.map(d => interpret_data(d)), wrap_n_times, wrap_n_times, to_2d_array, initialize_multidimensional);
                case "]":
                ...
                case "`":
                    return overload(data.data.map(d => interpret_data(d)), unimplemented, unimplemented, filter_code_points, filter_multidimensional);
                case "a":
                ...
                case "·":
                    return overload(data.data.map(d => interpret_data(d)), pass(range_inclusive, [0n]), pass(range_inclusive, [0]), code_point_range, array_range);
                case "Ȧ":
                ...
                case "Ċ":
                    return overload(data.data.map(d => interpret_data(d)), pass(divisible, [], [3n]), pass(divisible, [], [3]), pass(divisible, [], [3]), pass(length_divisible, [], [3]));
                case "Ḋ":
                ...
                case "Ė":
                    return overload(data.data.map(d => interpret_data(d)), unimplemented, unimplemented, levenshtein, map_other_item);
                case "Ḟ":
                ...
                case "Ḣ":
                    return overload(data.data.map(d => interpret_data(d)), unimplemented, unimplemented, rotate_180, swap_rows_columns);
                case "İ":
                ...
                case "Ṙ":
                    return overload(data.data.map(d => interpret_data(d)), pass(divisible, [], [5n]), pass(divisible, [], [5]), pass(divisible, [], [5]), pass(length_divisible, [], [5]));
                case "Ṡ":
                ...
                case "Ẋ":
                    return overload(data.data.map(d => interpret_data(d)), to_bytes, to_bytes, zero_pad_start, read_multidimensional);
                case "Ẏ":
                    return overload(data.data.map(d => interpret_data(d)), unimplemented, unimplemented, unimplemented, write_multidimensional);
                case "Ż":
                ...
                case "ȯ":
                    return overload(data.data.map(d => interpret_data(d)), unimplemented, unimplemented, remove_whitespace, default_filter);
                case "ṙ":
                    return Math.random();
                case "ṡ":
                ...
                case "Ạ":
                    return dynamic_wrapper((input_data) => {
                ...
                case "Ṛ":
                    return dynamic_wrapper((input_data) => {
                        ...
                        for (var i = 1; i < array.length; i++)
                            accumulator = interpret_data(data.data[0], [accumulator, array[i], i, input_data]);
                ...
                case "Ẉ":
                    return dynamic_wrapper((...input_data) => {
                        ...
                        return result;
                    }, interpret_data(data.data[1]), interpret_data(data.data[2]));
                case "Ỵ":
                    return dynamic_wrapper((input_position, input_data) => {
                        var array = to_array(input_data);
            ...
            if (data.string[0] == "‘")
                return string_notation(data.string.slice(1), data.data.map(d => interpret_data(d))).string;
            if (data.string[0] == "’")
            ...
            if (data.string[0] == "”") {
                ...
                while (position < data.string.length) {
                    substring = string_notation(data.string.slice(position), data.data.map(d => interpret_data(d)));
    ...
    return interpret_data(parse(tokenize(program)), [...inputs]);
};
