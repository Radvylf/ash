var interpret = (program = [], input = []) => {
    ...
    var interpret_data = (data, arguments = []) => {
        if (data.type == "operator") {
            switch (data.name) {
                ...
                case "Δ":
                    return overload(data.data.map(d => interpret_data(d)), distance, distance, levenshtein, fill);
                case "∞":
                    return Infinity;
                case "≡":
                ...
                case "U":
                    return overload(data.data.map(d => interpret_data(d)), bitwise_xor, bitwise_xor, shift_alpha, map_other_item);
                case "V":
                ...
                case "g":
                    return overload(data.data.map(d => interpret_data(d)), negative, negative, lower_case, sort_min);
                case "h":
                ...
                case "p":
                    return overload(data.data.map(d => interpret_data(d)), positive, positive, upper_case, sort_max);
                case "q":
                ...
                case "·":
                    return overload(data.data.map(d => interpret_data(d)), pass(range_inclusive, [0n]), pass(range_inclusive, [0]), array_range, array_range);
                case "Ȧ":
                ...
                case "Ė":
                    return overload(data.data.map(d => interpret_data(d)), unimplemented, unimplemented, pad_end, pad_end);
                case "Ḟ":
                ...
                case "Ṡ":
                    return overload(data.data.map(d => interpret_data(d)), unimplemented, unimplemented, pad_start, pad_start);
                case "Ṫ":
                ...
                case "Ẋ":
                    return overload(data.data.map(d => interpret_data(d)), to_bytes, to_bytes, pass(pad_start, [], ["0"]), read_multidimensional);
                case "Ẏ":
    ...
    return interpret_data(parse(tokenize(program)), [...input]);
};
